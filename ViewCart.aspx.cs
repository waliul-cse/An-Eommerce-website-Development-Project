...
public partial class ViewCart : System.Web.UI.Page
{
    ...
    protected void gvShoppingCart_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow && (e.Row.RowState == DataControlRowState.Normal || e.Row.RowState == DataControlRowState.Alternate))
        {
            ...
            imgProduct.ImageUrl = "~/Uploads/" + lblImage.Text;
    ...
    protected void gvShoppingCart_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "Remove")
        {
            ...
            Label imageUrl =gvr.FindControl("lblImage") as Label;
            int productId = Convert.ToInt32(e.CommandArgument);
            ShoppingCart.Instance.RemoveItem(productId, imageUrl.Text);
        }
        ...
        foreach (GridViewRow row in gvShoppingCart.Rows)
        {
            if (row.RowType == DataControlRowType.DataRow)
            {
                ...
                try
                {
                    ...
                    Label imageUrl = row.FindControl("lblImage") as Label;
                    int quantity = int.Parse(((TextBox)row.Cells[1].FindControl("txtQuantity")).Text);
                    ShoppingCart.Instance.SetItemQuantity(productId, imageUrl.Text, quantity);
                }
